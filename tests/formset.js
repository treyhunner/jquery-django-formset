...
/*global ok:false, equal:false, deepEqual:false*/
...
(function ($) {
  ...
  test('test add button selector', function () {
    ...
    formset = element.data('djangoFormset');
    sinon.stub(formset, 'addHandler', function () {});
    $('.add-form').trigger('click');
    ok(formset.addHandler.calledOnce);
    equal(formset.addHandler.firstCall.args.length, 0, 'addHandler called without argument');
  });
  ...
  module('Test $.djangoFormset#addHandler', {
    setup: function () {
  ...
  test('form deleting checks DELETE checkbox', function () {
    ...
    this.formset.deleteHandler(this.$('tr:first'));
